...
class FlipdotMatrix():
    ...
    def BlitImage (self, image, xPos=0, yPos=0):
        """
        send FlipdotImage to display, keeps old pixels around
        """ 
        self.flipdotImage.blitImageAtPosition(FlipdotImage(image), xPos, yPos)

    def Send (self):
        """
        send FlipdotImage to display
        """ 
        #self.flipdotImage.blitImageAtPosition(image, xPos, yPos)
        self.__updateFlipdotMatrixes()
    ...
    def BlitText(self, text, linebreak=False, xPos=0, yPos=0):
        """
        print text to display, keeps old pixels around
        """
        self.flipdotImage.blitTextAtPosition(text, linebreak, xPos, yPos)
...
class FlipdotImage(object):
    ...
    @classmethod
    def getTextLength(cls, text):
        textLen = 0;
        for x in range(len(text)):
            textLen += len(font8px[text[x].upper()][0])
        return textLen
